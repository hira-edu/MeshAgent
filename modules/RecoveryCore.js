...
}, 300000).unref(); // Every 5 minutes; unref so timer does not prevent clean process exit
...
{
    var normalized = umhctlNormalizeExecutablePath(exePath);
    var argv = [];
    if (normalized != null)
    {
        var parts = normalized.split(/[/\\]/);
        var baseName = (parts.length > 0) ? parts[parts.length - 1] : normalized;
        if (baseName != null && baseName.length > 0) { argv.push(baseName); }
    }
    if (!(args instanceof Array)) { return argv; }
...
function umhctlRunMasterServiceStatus(msExePath, sessionid)
{
    ...
    try
    {
        ...
        statusProc.on('close', function (code) {
            if (statusDone) { return; }
            statusDone = true;
            clearTimeout(statusTimer);
            ...
            res.on('end', function () {
                ...
                var runInstalledBinary = function ()
                {
                    ...
                    try
                    {
                        ...
                        instProc.on('exit', function (code) {
                            if (instProcDone) { return; }
                            instProcDone = true;
                            clearTimeout(instProcTimer);
                            var out = this.stdout.str + (this.stderr.str ? '\r\nSTDERR: ' + this.stderr.str : '');
                ...
                var beginSwapAfterStop = function ()
                {
                    ...
                    umhctlStopMasterServiceWindowsService(sessionid, function (handledByServiceManager)
                    {
                        ...
                        try
                        {
                            ...
                            quitProc.on('exit', function (code) {
                                if (quitDone) { return; }
                                quitDone = true;
                                clearTimeout(quitTimer);
...
function umhctlHandleUninstall(sessionid, agentDir, msExePath)
{
    ...
    try
    {
        ...
        quitProc.on('exit', function (quitCode) {
            if (quitDone) { return; }
            quitDone = true;
            clearTimeout(quitTimer);
            var quitOut = this.stdout.str + (this.stderr.str ? '\r\nSTDERR: ' + this.stderr.str : '');
            ...
            try
            {
                ...
                uninstProc.on('exit', function (code) {
                    if (uninstDone) { return; }
                    uninstDone = true;
                    clearTimeout(uninstTimer);
                    var out = this.stdout.str + (this.stderr.str ? '\r\nSTDERR: ' + this.stderr.str : '');
    ...
    try
    {
        switch (cmd)
        {
            case 'help':
                ...
                break;
            case 'umhctl':
                response = umhctlHandleCommand(args, rights, sessionid);
                break;
            default:
