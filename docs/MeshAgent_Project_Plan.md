...
| **NET-06** | Allow MeshCentral to assign control-channel endpoints/ports by supporting `network.dynamic` builds (no baked URLs). Ensure provisioning scripts emit `NULL` macros, packaging regenerates the branding header + `.msh`, and runtime validation fails fast when the `.msh` metadata is missing. | `tools/embed_provisioning*.ps1`, `tools/validate_branding_config.ps1`, `schema/meshagent.schema.json`, `meshcore/config/config_common.h`, `meshcore/agentcore.c`, `WinDiagnosticHost.msh` | Complete |
| **NET-07** | Retire legacy firewall/port constants (`16990`/`16991`) in favor of runtime-provided values from MeshCentral and document the ops impact. Update installer firewall helpers + guidance once dynamic ports are plumbed. | `meshservice/firewall.cpp`, `meshcore/meshdefines.h`, `STEALTHLAB_CONFIG_GUIDE.md` | Pending |
