...
if (-not (Test-SvchostPayload -Path $exeX64)) {  # ← CONTROL POINT #7
    throw "Issued build does not contain embedded svchost payload payload resource: $exeX64"
}
```

**Control Point #7: Embedded DLL Verification**
- **Function:** `Test-SvchostPayload` (from ResourceProbe.ps1)
- **Checks:** EXE contains embedded embedded svchost payload resource
...
- **Contains:** `embedded svchost payload` resource (embedded DLL)
...
```
bundle_resources.rc:
    embedded svchost payload RCDATA "embedded/svchost_payload.dll"
```
