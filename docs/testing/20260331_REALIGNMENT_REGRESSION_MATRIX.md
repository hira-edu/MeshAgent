...
| svchost install integrity | service registration uses the intended quoted local path, expected `ServiceDll`, expected host-group assumptions, and no remote-share payload path | REQUIRED | native CLI plus SCM/registry validation | `docs/testing/evidence/advanced/<ts>_svchost_install_integrity/` | yes |
...
| Localhost relay hardening | loopback listener ACLs, control-plane authorization, restart/update continuity, and retargeting-denial behavior all match policy | REQUIRED | native probe plus policy validation | `docs/testing/evidence/advanced/<ts>_localhost_relay_hardening/` | yes |
...
| Session-broker IPC integrity | Session 0 service and Session 1 bridge communicate only through the authenticated IPC contract, with correlation IDs, bounded token-handle lifecycle, and no generic command execution semantics | REQUIRED | native probe plus audited bridge run | `docs/testing/evidence/advanced/<ts>_session_broker_ipc/` | yes |
| Privilege boundary | privileged operations remain explicit and bounded; no persistent generic elevated shell or reusable full-elevation child context exists | REQUIRED | native policy validation plus negative tests | `docs/testing/evidence/advanced/<ts>_privilege_boundary/` | yes |
...
| Protected-boundary failure semantics | when Windows security boundaries block capture or input, the system emits explicit failure evidence and no bypass attempt occurs | REQUIRED | native probe plus negative harness | `docs/testing/evidence/advanced/<ts>_protected_boundary_failure/` | yes |
...
| Audit log quality | security, operational, and transaction/evidence logs are separated appropriately, carry correlation IDs, and use protected transport on export | REQUIRED | harness plus evidence inspection | `docs/testing/evidence/advanced/<ts>_audit_log_quality/` | yes |
