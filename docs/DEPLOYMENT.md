...
- Package-driven Windows updates adopt the staged package provisioning identity (`MeshID`, `ServerID`, `MeshServer`) while preserving the installed `NodeID`; binary-only updates continue to retain the installed provisioning identity.
